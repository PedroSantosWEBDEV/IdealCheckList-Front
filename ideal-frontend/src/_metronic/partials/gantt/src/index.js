...
var ganttDateRange = function ganttDateRange(tasks, viewMode, preStepsCount) {
  ...
  switch (viewMode) {
    ...
    case ViewMode.Week:
      ...
      newStartDate = addToDate(getSunday(newStartDate), -7 * preStepsCount, "day");
...
var getSunday = function getSunday(date) {
  var day = date.getDay();
...
var getWeekDates = function getWeekDates(date) {
  ...
  const firstDateOfWeek = new Date(tmpDate.setDate(date.getDate()))
  const lastDayOfWeek = tmpDate.getDate() + 6
  const lastDateOfWeek = new Date(tmpDate.setDate(lastDayOfWeek))
  
  const formatedFirstDateOfWeek = formatDate(firstDateOfWeek)
  const formatedLastDateOfWeek = formatDate(lastDateOfWeek)
  // // console.log('firstDateOfWeek: ', firstDateOfWeek)

  return formatedFirstDateOfWeek + ' - ' + formatedLastDateOfWeek
};
...
var TaskListHeaderDefault = function TaskListHeaderDefault(_ref) {
  ...
  }, "\xA0"+useIntl().formatMessage({id: 'SCHEDULE.LABEL.NAME'})), React__default.createElement("div", {
  ...
  }, "\xA0"+useIntl().formatMessage({id: 'SCHEDULE.LABEL.FROM'})), React__default.createElement("div", {
    className: styles.ganttTable_HeaderSeparator,
    style: {
      height: headerHeight * 0.5,
  ...
  }, "\xA0"+useIntl().formatMessage({id: 'SCHEDULE.LABEL.TO'}))));
};
...
var StandardTooltipContent = function StandardTooltipContent(_ref2) {
  ...
  }, task.name + ": " + ("0" + task.start.getDate()).slice(-2) + "/" + ("0" + (task.start.getMonth() + 1)).slice(-2) + "/" + task.start.getFullYear() + " - " + ("0" + task.end.getDate()).slice(-2) + "/" + ("0" + (task.end.getMonth() + 1)).slice(-2) + "/" + task.end.getFullYear()), React__default.createElement("p", {
    className: styles$2.tooltipDefaultContainerParagraph
  }, useIntl().formatMessage({id: 'SCHEDULE.LABEL.DURATION'}) + ~~((task.end.getTime() - task.start.getTime()) / (1000 * 60 * 60 * 24)) + " " + useIntl().formatMessage({id: 'SCHEDULE.LABEL.DAYS'})), React__default.createElement("p", {
    className: styles$2.tooltipDefaultContainerParagraph
  }, useIntl().formatMessage({id: 'SCHEDULE.LABEL.PROGRESS'}) + task.progress + " %"));
};
...
var Calendar = function Calendar(_ref) {
  ...
  var getCalendarValuesForWeek = function getCalendarValuesForWeek() {
    ...
    for (var i = dates.length - 1; i >= 0; i--) {
      ...
      var bottomValue = getWeekDates(date);
      bottomValues.push(React__default.createElement("text", {
        key: date.getTime(),
        y: headerHeight * 0.8,
        x: columnWidth * (i + +rtl),
